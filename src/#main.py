...
def main_extract():
    ...
    if args.train or args.validate:
        ...
        for i in range(0, len(data)):
            fnInfer = ('/Users/alone_walker/Desktop/htr/SimpleHTR/my{}.png'.format(i))
            #infer(model, fnInfer)
            fnImg = fnInfer
            img = preprocess(cv2.imread(fnImg, cv2.IMREAD_GRAYSCALE), Model.imgSize)
            batch = Batch(None, [img])
            ...
            print(f'Recognized: {recognized[0]}')
            print(f'Probability: {probability[0]}')
            os.remove('/Users/alone_walker/Desktop/htr/SimpleHTR/my{}.png'.format(i))
        print(final_string)
        trial_string = trial_string.join(str(item) for item in final_string)

        print(trial_string)
        #os.remove('G:\location htr img\my{}.png'.format(i))
#for i in range(0,len(data)):
